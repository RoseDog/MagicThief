...
public class SimpleSmoothModifier : MonoModifier {
	...
	public double maxSegmentLength = 2F;
	...
	public List<Vector3> CurvedNonuniform (List<Vector3> path) {
		...
		for (int i=0;i<path.Count-1;i++) {
			...
			double dist = (path[i]-path[i+1]).magnitude;
			...
			for (double t=0;t<=dist;t+=maxSegmentLength) {
				pointCounter++;
			}
		}
		...
		for (int i=0;i<path.Count-1;i++) {
            ...
            Vector3 startVel = startVel1 * (float)dist * factor;
            Vector3 endVel = endVel1 * (float)dist * factor;
			...
			double onedivdist = 1F / dist;
			
			for (double t=0;t<=dist;t+=maxSegmentLength) {
				
				double t2 = t * onedivdist;
	...
	public static Vector3 GetPointOnCubic (Vector3 a, Vector3 b, Vector3 tan1, Vector3 tan2, double t) {
        ...
        return (float)h1 * a +                   	 // multiply and sum all funtions
                    (float)h2 * b +                   	 // together to build the interpolated
                    (float)h3 * tan1 +                	 // point along the curve.
                    (float)h4 * tan2;
	...
	public List<Vector3> SmoothSimple (List<Vector3> path) {
		...
		if (uniformLength) {
			int numSegments = 0;
			maxSegmentLength = maxSegmentLength < 0.005 ? 0.005 : maxSegmentLength;
			
			for (int i=0;i<path.Count-1;i++) {
				double length = Vector3.Distance (path[i],path[i+1]);

                numSegments += Mathf.FloorToInt((float)length / (float)maxSegmentLength);
			}
			...
			for (int i=0;i<path.Count-1;i++) {
                ...
                int numSegmentsForSegment = Mathf.FloorToInt(((float)length + carry) / (float)maxSegmentLength);
				
				double carryOffset = carry/length;
				...
				for (int q=0;q<numSegmentsForSegment;q++) {
                    ...
                    subdivided.Add(dir * ((float)q / numSegmentsForSegment - (float)carryOffset) + path[i]);
					c++;
				}

                carry = ((float)length + carry) % (float)maxSegmentLength;
			}
			...
			for (int it = 0; it < iterations; it++) {
				...
				for (int i=1;i<subdivided.Count-1;i++) {
                    ...
                    subdivided[i] = Vector3.Lerp(tmp, (prev + subdivided[i + 1]) / 2F, (float)strength);
